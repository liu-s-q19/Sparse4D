...
class Sparse4DHead(BaseModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.loss_scs = build(loss_scs, LOSSES)
    ...
    def forward(
        ...
        if dn_metas is not None:
            ...
            score = [x[:, :num_free_instance] for x in score]
    ...
    def loss(self, model_outs, data, feature_maps=None):
        ...
        score = model_outs["score"]
        output = {}
        for decoder_idx, (cls, reg, qt, sc) in enumerate(
            zip(cls_scores, reg_preds, quality, score)
        ):
            reg = reg[..., : len(self.reg_weights)]
            cls_target, reg_target, reg_weights, sc_target = self.sampler.sample(
                cls,
                reg,
            ...
            sc = sc.flatten(end_dim=1)
            # sc = sc.squeeze(-1)
            sc_target = sc_target.flatten(end_dim=1)
            scs_loss = self.loss_scs(sc, sc_target, avg_factor=num_pos)
            ...
            output[f"loss_scs_{decoder_idx}"] = scs_loss
